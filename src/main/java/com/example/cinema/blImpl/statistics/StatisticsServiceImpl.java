    ...
    public ResponseVO getMoviePlacingRateByDate(Date date) {
        try {
            ...
            for(MovieScheduleTime movieScheduleTime:movieScheduleTimeList){
                ...
                List<ScheduleItem> scheduleItemList=scheduleServiceForBl.selectScheduleByMovieIdAndDate(movieScheduleTime.getMovieId(),date,getNumDayAfterDate(date,1));
                for (ScheduleItem scheduleItem:scheduleItemList){
                    ...
                    List<Ticket> tickets=ticketServiceForBl.selectTicketsBySchedule(scheduleItem.getId());
